...
<head>
    ...
    <title>Delivery Line Management</title>
    ...
    <style>
        ...
        /* Hide the ID column but keep it in the DOM */
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-header">
                        <i class="fas fa-plus-circle me-2"></i>Create New Delivery Line
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            <i class="fas fa-list me-2"></i>Delivery Line List
                        </div>
                        <div class="d-flex">
        ...
        <div class="modal fade" id="deleteLineModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Are you sure you want to delete this delivery line? This action cannot be undone.</p>
...
function loadLines() {
    ...
    // Load all data at once since we can't modify the API for server-side search
    fetch('api/clusters_api.php')
        .then(response => {
            if (!response.ok) {
            ...
            if (data.status === 'success') {
                ...
                if (searchQuery) {
                    filteredLines = allLines.filter(line => 
                        (line.clusters_name && line.clusters_name.toLowerCase().includes(searchQuery.toLowerCase())) ||
                        (line.clusters_location && line.clusters_location.toLowerCase().includes(searchQuery.toLowerCase())) ||
                        (line.clusters_pincode && line.clusters_pincode.toLowerCase().includes(searchQuery.toLowerCase())) ||
                        (line.zone_name && line.zone_name.toLowerCase().includes(searchQuery.toLowerCase())) ||
                        (line.dcm_name && line.dcm_name.toLowerCase().includes(searchQuery.toLowerCase())) ||
                        (line.cluster_name && line.cluster_name.toLowerCase().includes(searchQuery.toLowerCase()))
                    );
                }
                    ...
                    linesTableBody.innerHTML = `
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                <i class="fas fa-info-circle me-2"></i>No delivery lines exist
                            </td>
                        </tr>
                    `;
                ...
                const paginatedLines = filteredLines.slice(startIndex, startIndex + perPage);
                
                renderLines(paginatedLines);
                renderPagination({
                    total_pages: Math.ceil(filteredLines.length / perPage),
                    current_page: currentPage
                });
            } else {
                showResponseMessage('error', data.message || 'Failed to load delivery lines');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showResponseMessage('error', 'Failed to load delivery lines. Please try again.');
        })
        .finally(() => {
            ...
            function renderLines(lines) {
                ...
                if (!lines || lines.length === 0) {
                    linesTableBody.innerHTML = `
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                <i class="fas fa-info-circle me-2"></i>No delivery lines found
                            </td>
                        </tr>
                    `;
                    return;
                ...
                lines.forEach(line => {
                    ...
                    // The ID column is hidden by CSS but still present in the DOM
                    row.innerHTML = `
                        <td>${line.clusters_id}</td>
                        <td>${line.clusters_name}</td>
                        <td>${line.clusters_location}</td>
                        <td>${line.clusters_pincode}</td>
                        <td>${line.zone_name || 'N/A'}</td>
                        <td>${line.dcm_name || 'N/A'}</td>
                        <td>${line.cluster_name || 'N/A'}</td>
                        <td class="action-btns">
                            <button class="btn btn-sm btn-outline-primary edit-line" data-id="${line.clusters_id}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger delete-line" 
                                    data-id="${line.clusters_id}" 
                                    data-name="${line.clusters_name}">
...
function renderPagination(meta) {
    ...
    if (!validateLineName() || !validateLineLocation() || !validateLinePincode() || !zoneId || !dcmId) {
        ...
        return;
                ...
                fetch('api/clusters_api.php', {
                    ...
                    body: JSON.stringify({
                        clusters_name: lineName,
                        clusters_location: lineLocation,
                        clusters_pincode: linePincode,
                        cluster_id: 'CID_003', // Fixed cluster ID
                        zone_id: zoneId,
                        dcm_id: dcmId
                    })
                })
                ...
                .catch(error => {
                    console.error('Error:', error);
                    showResponseMessage('error', 'Failed to create delivery line. Please try again.');
                })
                .finally(() => {
            ...
            function showEditLineModal(lineId) {
                fetch(`api/clusters_api.php?clusters_id=${lineId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.status === 'success') {
                            const line = data.data;
                            
                            document.getElementById('editLineId').value = line.clusters_id;
                            document.getElementById('editLineName').value = line.clusters_name;
                            document.getElementById('editLineLocation').value = line.clusters_location;
                            document.getElementById('editLinePincode').value = line.clusters_pincode;
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        showResponseMessage('error', 'Failed to load delivery line details. Please try again.');
                    });
            }
    ...
    if (!validateEditLineName() || !validateEditLineLocation() || !validateEditLinePincode() || !zoneId || !dcmId) {
        ...
        return;
                ...
                .then(data => {
                    if (data.status === 'success') {
                        showResponseMessage('success', 'Delivery line updated successfully');
                        loadLines();
                ...
                .catch(error => {
                    console.error('Error:', error);
                    showResponseMessage('error', 'Failed to update delivery line. Please try again.');
                })
                .finally(() => {
                    spinner.classList.add('d-none');
                });
            ...
            function deleteLine() {
                ...
                .catch(error => {
                    console.error('Error:', error);
                    showResponseMessage('error', 'Failed to delete delivery line. Please try again.');
                })
                .finally(() => {
                    spinner.classList.add('d-none');
                });
                    ...
                    element.innerHTML = `
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                ...
                                <p class="mt-2">Loading delivery lines...</p>
                            </td>
                        </tr>
                    `;
                }
